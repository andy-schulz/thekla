import {setBrowserStackSessionName, standardTheklaCapabilities, standardTheklaServerConfig} from "@thekla/support";
...
describe(`The annotation`, (): void => {
    ...
    describe(`without an annotation parameter`, (): void => {
        ...
        beforeAll(async (): Promise<void> => {
            searchInput = await client.findElement('css selector', 'body #exampleEmail')
        });

        it(`should not be performed on the element 
        - (test case id: 9f5d9b10-f864-45ed-8d85-8ce1c661300f)`, async (): Promise<void> => {
            await client.navigateTo('http://framework-tester.test-steps.de');
            await AnnotatorWdio.highlight(searchInput)
        });
    });
